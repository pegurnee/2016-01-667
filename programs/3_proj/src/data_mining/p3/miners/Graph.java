...
public class Graph {
    ...
    public double computeSumSquaredErrorOfCluster(int clusterNumber) {
        final ArrayList<Record> clusteredRecords = new ArrayList<>();
        ...
        final Record centroid = this.getCentroid(clusteredRecords);

        double sum = 0.0;

        // find the squared sum of the distances
        for (final Record record : clusteredRecords) {
    ...
    public void display(String outputFile) throws IOException {
        ...
        this.writeRecordsOrderedByCluster(outFile);
    ...
    public void display(String outputFile, boolean ordered) throws IOException {
        ...
        if (ordered) {
            ...
            this.writeRecordsUnordered(outFile);
        }
    ...
    public int getNumClusters() {
        final Set<Integer> clusterOptions = new HashSet<>();
    ...
    public void load(String inputFile) throws FileNotFoundException {
        ...
        // file comes with a label saying if the data is normalized or not
        final boolean normal = "normal".equals(inFile.next());

        // if it is not normalized, get the ranges for each data set
        if (!normal) {
    ...
    private void writeErrorRates(PrintWriter outFile) {
        final int numClusters = this.getNumClusters();
